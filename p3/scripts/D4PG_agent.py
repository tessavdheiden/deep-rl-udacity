...
from scripts.buffers import ReplayBuffer
from scripts.models import ActorNet, CriticNet
...
class MAD4PG_Net:
    ...
    def __init__(self, env, args, state_size, action_size, agent_count):
        """
    ...
    def initialize_memory(self, pretrain_length, env):
        ...
        obs = torch.from_numpy(env.env_info.vector_observations).float()
    ...
    def save_agents(self, folder='saved_weights', name='d4pg'):
        # save models for local actor and critic of each agent
        for i, agent in enumerate(self.agents):
            torch.save(agent.actor.state_dict(),  "{}/{}_checkpoint_actor_agent_{}.pth".format(folder, name, i))
            torch.save(agent.critic.state_dict(), "{}/{}_checkpoint_critic_agent_{}.pth".format(folder, name, i))
